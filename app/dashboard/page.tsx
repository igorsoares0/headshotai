...
import { orders, orderStates, gallery } from "@/lib/data";

const summary = [
  { label: "Credits left", value: "82", hint: "of 120" },
  { label: "Orders", value: "4", hint: "1 in progress" },
  { label: "Headshots", value: "160", hint: "across packs" },
  { label: "Favorites", value: "12", hint: "saved" },
];

// active order = the GENERATING one
const active = orders.find((o) => o.status === "GENERATING")!;

export default function DashboardHome() {
  const currentIdx = orderStates.indexOf(
    active.status as (typeof orderStates)[number],
  );
  ...
  return (
    <>
      ...
      <div className="space-y-8 px-5 py-8 sm:px-8">
        ...
        <div className="rounded-card border border-line bg-ink p-6 text-paper sm:p-8">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <p className="kicker text-muted-dark">Order {active.id}</p>
              <h2 className="mt-1 text-xl font-bold tracking-tight">
                {active.pkg} pack — generating your headshots
              </h2>
            </div>
          ...
          {/* stepper */}
          <ol className="mt-8 grid gap-3 sm:grid-cols-7">
                ...
                <li key={state} className="flex items-center gap-3 sm:flex-col sm:items-start">
                  <span
                    className={`grid h-7 w-7 shrink-0 place-items-center rounded-full text-xs font-bold ${
                        ...
                        : now
                          ? "bg-paper text-ink"
                          : "border border-line-invert text-muted-dark"
                    }`}
                  >
                    {done ? "✓" : i + 1}
                  </span>
                  <span
                    className={`font-mono text-[10px] uppercase tracking-wide ${
                      now ? "text-paper" : "text-muted-dark"
                    }`}
                  >
                    {state.replace(/_/g, " ")}
                  </span>
          ...
          <div className="mt-6 h-1.5 overflow-hidden rounded-full bg-ink-raised">
            <div
              ...
              style={{ width: `${((currentIdx + 0.5) / orderStates.length) * 100}%` }}
            />
          </div>
        </div>
          ...
          >
            ...
            <div className="mt-10">
              ...
              <p className="mt-2 text-sm text-muted">
                Upload selfies and generate a fresh set of styles from your
                identity model.
              </p>
            </div>
          </Link>
